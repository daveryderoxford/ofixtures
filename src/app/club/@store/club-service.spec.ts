...
describe('ClubService', () => {
  ...
  let mockFixturesService: { fixtures: WritableSignal<Fixture[]> };

  beforeEach(() => {
    // Create a mock for FixturesService
    mockFixturesService = {
      fixtures: signal(mockFixtures), // Return a signal with mockFixtures
    };
  ...
  describe('clubs computed signal', () => {
    ...
    it('should handle an empty fixtures list', () => {
      mockFixturesService.fixtures.set([]);
    ...
    it('should filter out fixtures with empty or whitespace-only club names', () => {
      ...
      mockFixturesService.fixtures.set(fixturesWithEmptyClub);
      const clubs = service.clubs();
  ...
  describe('fixtures computed signal', () => {
    it('should return fixtures for the selected club', () => {
    ...
    it('should return an empty array if no club is selected', () => {
      service.selected.set(null);
      expect(service.fixtures().length).toBe(0);
    });

    it('should return an empty array if the selected club has no fixtures (or does not exist)', () => {
