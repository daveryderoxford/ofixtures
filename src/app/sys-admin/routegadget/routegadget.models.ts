...
export type RGFormat = 'a' | 'b';
export type RGType = 'I' | 'N' | 'R' | 'L' | 'T';

export interface RGSiteEvents extends RGSite {
   events: RGEvent[];
}

export interface RGEventRaw {
   id: string;
   mapid: string;
   format: RGFormat;
   name: string;
...
export interface RGEvent {
   ...
   mapid: number;
   mapfile: string;
   worldFile: any; // You might want to create a proper Worldfile interface here too
}
